...
public class ExtendJavascriptInterface<T>{
    ...
    public ExtendJavascriptInterface(T object, String injectedName) {
        Class injectedCls=object.getClass();
        obj=object;
        try {
            ...
            for (Method method : methods) {
                ...
                boolean flag=false;//寻找注解
                Annotation[] annotations = method.getAnnotations();
                for(Annotation annotation : annotations) {
                    if(annotation.annotationType().getName().equals("android.webkit.JavascriptInterface")){
                        flag=true;
                    }
                }
                if(flag) {
            ...
            sb.append(mInjectedName);
            sb.append(" call error, message:miss method name\"}var e=[];for(var h=1;h<f.length;h++){var c=f[h];var j=typeof c;e[e.length]=j;if(j==\"function\"){var d=a.queue.length;a.queue[d]=c;f[h]=d}}var g=JSON.parse(");
            sb.append("prompt");
            sb.append("(JSON.stringify({method:f.shift(),types:e,args:f})));if(g.code!=200){throw\"");
            sb.append(mInjectedName);
            ...
            //Log.e("test",mPreloadInterfaceJS);
        } catch(Exception e){
    ...
    private String genJavaMethodSign (Method method) {
        ...
        for (int k = 0; k < len; k++) {
            ...
            if (cls == String.class) {
                ...
                sign += "_O";
            } else if (cls == ExtendJavascriptCallback.class) {
                sign += "_F";
            } else {
                sign += "_P";
            }
            ...
            try {
                ...
                for (int k = 0; k < len; k++) {
                    ...
                    if ("string".equals(currType)) {
                        ...
                        sign += "_F";
                        values[k] = new ExtendJavascriptCallback(webView, mInjectedName, argsVals.getInt(k));
                    } else {
                        sign += "_P";
                    }
                }
                ...
                if (numIndex > 0) {
                    ...
                    while (numIndex > 0) {
                        ...
                        if (currCls == int.class) {
                            values[currIndex-1] = argsVals.getInt(currIndex - 1);
                        } else if (currCls == long.class) {
                            //WARN: argsJson.getLong(k + defValue) will return a bigger incorrect number
                            values[currIndex-1] = Long.parseLong(argsVals.getString(currIndex - 1));
                        } else {
                            values[currIndex-1] = argsVals.getDouble(currIndex - 1);
                        }
                        numIndex /= 10;
                    }
                }
